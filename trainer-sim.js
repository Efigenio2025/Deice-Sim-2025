...
(function(){
  // ===== constants =====
  const WRITE_TOKEN = 'wR1te_Train3r_XyZ_90210';  // replace if rotated
  ...
  function tailToPhonetic(tail){ const m=String(tail||'').toUpperCase().match(/\bN[0-9A-Z]{3,}\b/); if(!m) return null; return m[0].split('').map(ch=>NATO[ch]||ch).join(' '); }
  ...
  function gradeDetails(expected, heard){ const eSet=new Set(tokens(expected)), hSet=new Set(tokens(heard)); const matched=[...eSet].filter(w=>hSet.has(w)).length; return { matchedCount: matched, total: eSet.size }; }
  ...
  function primeTranscriptForStep(st){
    ...
    if(st?.role==='Iceman'){
      ...
      expLine.innerHTML = expected.split(' ').map(w=>`<span class="tok miss">${w}</span>`).join(' ');
...
function close(){
  ...
  function makeRecognizer(){
    ...
    const rec = new R();
    rec.lang='en-US';
    rec.continuous=false;        // single window; we re-start as needed
    rec.interimResults=true;     // STREAMING partials
    rec.maxAlternatives=1;
    return rec;
  }

  // Stream listen for up to windowMs; update Live Input; return best final text/score
  async function listenWindowStream(step, windowMs){
    return new Promise((resolve) => {
      ...
      rec.onerror = (e)=>{ if(statusEl) statusEl.textContent=`Mic error: ${e.error||e}`; resolve({ score:bestScore, heard:bestHeard }); };
      rec.onend = ()=>{
        if(ended){
  ...
  async function playCaptainAndAwait(st){
    ...
    return new Promise((resolve)=>{
      ...
      try{
        a.pause(); a.currentTime=0; a.src=(/^https?:\/\//i.test(audioSrc))? audioSrc : `/audio/${audioSrc}`;
        a.onended=()=> resolve();
        const p=a.play(); setText($('status'),'Playing Captain line…'); if(p&&p.catch) p.catch(()=> resolve());
      }catch{ resolve(); }
    });
  }
  ...
  // ===== Wire UI =====
  const SCENARIOS_API_HINT = 'Use Safari on iOS • HTTPS required';
  const SCENARIOS=[]; // already declared up top; kept for clarity
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ensureEmpGate();
    loadScenarios();
    setText($('status'), 'Idle. ' + SCENARIOS_API_HINT);
